...
class DeformableConvolutional(nn.Module):
    def __init__(self, filters_in, filters_out, norm=None, activate=None):
        super(DeformableConvolutional, self).__init__()

        self.norm = norm
        self.activate = activate

        self.__conv = DeformableConv2d(in_channels=filters_in, out_channels=filters_out)
        if norm:
            assert norm in norm_name.keys()
            if norm == "bn":
                self.__norm = norm_name[norm](num_features=filters_out)
...
if __name__ == "__main__":
    ...
    c = DeformableConvolutional(3,3,"bn",'leaky')
    oup = c(a)
    print(oup.shape)
