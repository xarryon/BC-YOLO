...
def nms(bboxes, score_threshold, iou_threshold, sigma=0.3, method='nms'):
    ...
    for cls in classes_in_img:
        ...
        while len(cls_bboxes) > 0:
            ...
            cls_bboxes = np.concatenate([cls_bboxes[: max_ind], cls_bboxes[max_ind + 1:]])#排除掉max的那个
            iou = iou_xyxy_numpy(best_bbox[np.newaxis, :4], cls_bboxes[:, :4])
            assert method in ['nms', "nms_d", 'soft-nms', "soft-nms+"]
            ...
            if method == 'nms':
                ...
                best_ct_x = (best_bbox[2] + best_bbox[0])/2
                best_ct_y = (best_bbox[3] + best_bbox[1])/2
                box_ct_x = (cls_bboxes[:, 2] + cls_bboxes[:, 0])/2
                box_ct_y = (cls_bboxes[:, 3] + cls_bboxes[:, 1])/2
                ct_dis = (best_ct_x - box_ct_x)**2 + (best_ct_y - box_ct_y)**2
                x_min = np.minimum(best_bbox[0], cls_bboxes[:, 0])
                x_max = np.maximum(best_bbox[2], cls_bboxes[:, 2])
                y_min = np.minimum(best_bbox[1], cls_bboxes[:, 1])
                y_max = np.maximum(best_bbox[3], cls_bboxes[:, 3])
                cor_dis = (y_max - y_min)**2 + (x_max - x_min)**2
                
                ratio = ct_dis/cor_dis
                iou_mask = iou - ratio > iou_threshold
                weight[iou_mask] = 0.0
                ...
                ratio = np.exp(ct_dis/cor_dis)
                weight = np.exp(-(1.0 * iou ** 2 / (sigma*ratio)))
