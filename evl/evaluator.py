...
#            img_path = os.path.join(self.val_data_path, img_ind+'.jpg')
            ...
            if bboxes_prd.shape[0]!=0 and self.__visiual and self.__visual_imgs < 1200:
                boxes = bboxes_prd[..., :4]
    ...
    def get_bbox(self, img, multi_test=False, flip_test=False):
        if multi_test:
            test_input_sizes = range(448, 640, 128)
            bboxes_list = []
    ...
    def __calc_APs(self, iou_thresh=0.5, use_07_metric=False):
        ...
        fps = sum(self.inf_time)/4.952
        nms_time = sum(self.nms_time)/4.952
