...
import random
import argparse
from evl.evaluator import Evaluator
from utils.tools import *
import config.yolov3_config_voc_tunning as cfg
...
class Trainer(object):
    def __init__(self,  weight_path, resume, gpu_id):
        ...
        self.train_dataset = data.VocDataset(anno_file_type="5shot_box/train_novel", img_size=cfg.TRAIN["TRAIN_IMG_SIZE"])
        ...
        for para in self.yolov3._Yolov3__backnone.parameters():
            para.requires_grad = False
        
        #for para in self.yolov3._Yolov3__spp.parameters():
        #    para.requires_grad = False

        for para in self.yolov3._Yolov3__fpn.parameters():
            para.requires_grad = False
        
        for para in self.yolov3._Yolov3__head_s.parameters():
            para.requires_grad = False

        for para in self.yolov3._Yolov3__head_m.parameters():
            para.requires_grad = False

        for para in self.yolov3._Yolov3__head_l.parameters():
            para.requires_grad = False

        #self.yolov3._Yolov3__backnone.requires_grad_ = False
        #self.yolov3._Yolov3__fpn.requires_grad_ = False
        
        #self.optimizer = optim.SGD(self.yolov3.parameters(), lr=cfg.TRAIN["LR_INIT"],
        #                           momentum=cfg.TRAIN["MOMENTUM"], weight_decay=cfg.TRAIN["WEIGHT_DECAY"])
        
        self.optimizer = optim.SGD(filter(lambda p:p.requires_grad, self.yolov3.parameters()), lr=cfg.TRAIN["LR_INIT"],
                                    momentum=cfg.TRAIN["MOMENTUM"], weight_decay=cfg.TRAIN["WEIGHT_DECAY"])
    ...
    def __load_model_weights(self, weight_path, resume):
        if resume:
            ...
            self.yolov3.load_state_dict(chkpt)
    ...
    def __save_model_weights(self, epoch, mAP):
        if mAP > self.best_mAP:
            self.best_mAP = mAP
        best_weight = os.path.join(os.path.split(self.weight_path)[0], "best_tunning.pt")
        last_weight = os.path.join(os.path.split(self.weight_path)[0], "last_tunning.pt")
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, (imgs, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes)  in enumerate(self.train_dataloader):
                ...
                p, p_d = self.yolov3(imgs, 'tunning')
                p_bs, p_d_bs = self.yolov3(imgs, 'base')

                #p, p_d = self.yolov3(imgs, 'base')
                
                loss, loss_giou, loss_conf, loss_cls = self.criterion(p, p_d, label_sbbox, label_mbbox,
                                                  label_lbbox, sbboxes, mbboxes, lbboxes, p_bs, p_d_bs)
            ...
            if epoch >= 40:
                ...
                #novel_list = ['bird', 'bus', 'cow', 'motorbike', 'sofa']
                novel_list = ['aeroplane', 'bottle','cow', 'horse', 'sofa']
                #novel_list = ['boat', 'cat', 'motorbike', 'sheep', 'sofa']
                
                with torch.no_grad():
                    APs, fps, nms_time = Evaluator(self.yolov3).APs_voc()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--weight_path', type=str, default='weight/2/best.pt', help='weight file path')
