...
class VocDataset(Dataset):
    ...
    def __load_annotations(self, anno_type):
        ...
        #assert anno_type in ['train', 'train_base', 'train_novel', 'test'], "You must choice one of the 'train' or 'test' for anno_type parameter"
        anno_path = os.path.join(cfg.PROJECT_PATH, 'data', anno_type+".txt")
    ...
    def __creat_label(self, bboxes):
        ...
        label = [np.zeros((int(train_output_size[i]), int(train_output_size[i]), anchors_per_scale, 6+self.num_classes))
                                                                      for i in range(3)]#13 13 3 6+classes
        for i in range(3):
            label[i][..., 5] = 1.0
        ...
        for bbox in bboxes:#6
            bbox_coor = bbox[:4]
            bbox_class_ind = int(bbox[4])#class类别
            bbox_mix = bbox[5]
            ...
            bbox_xywh_scaled = 1.0 * bbox_xywh[np.newaxis, :] / strides[:, np.newaxis]#3种对应不同尺度下的gt_box
