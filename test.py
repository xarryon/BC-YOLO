...
class Tester(object):
    ...
    def test(self):
        ...
        if self.__eval:
            ...
            with torch.no_grad():
                APs, fps, nms_time = Evaluator(self.__model).APs_voc(self.__multi_scale_test, self.__flip_test)
                ...
                mAP = mAP / (self.__num_class - 15)
                bAP = bAP / (self.__num_class - 5)
                AP = AP / self.__num_class

                print('nAP:%g' % (mAP))
                print('bAP:%g' % (bAP))
                print('AP:%g' % (AP))
                print("fps: %.1f ms" %(fps))
                print("nms_time: %.1f ms" %(nms_time))


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--weight_path', type=str, default='weight/3/10shot/best_tunning.pt', help='weight file path')
