...
from .dropblock import DropBlock, AttentiveDropBlock


class Yolo_head(nn.Module):
    def __init__(self, nC, anchors, stride, in_channel, out_channel, dconv=False):
        super(Yolo_head, self).__init__()
        
        #if dconv == True:
        #    self.__dconv = DeformableConvolutional(filters_in=in_channel, filters_out=in_channel, norm='bn', activate='leaky')
        #else:
        #    self.__dconv = nn.Identity()
        ...
        self.__conv_1 = Convolutional(filters_in=in_channel*2, filters_out=out_channel, kernel_size=1,
                                       stride=1, pad=0)
        ...
        #self.dropblock = DropBlock(block_size=5)
